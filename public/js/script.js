...
// ============================================================
// ========== NEW FEATURES (Infinite Scroll, Lightbox, Recently Viewed) ==========
// ============================================================
// These additions do NOT alter any existing code – they only add new capabilities.

// --- INFINITE SCROLL (replaces pagination with auto-load) ---
let isLoadingMore = false;
let hasMorePages = true;

// Override changePage to also support infinite scroll (but keep original for manual pagination)
...
async function loadNextPage() {
  ...
  try {
    ...
    if (newHouses.length === 0) {
      ...
      totalPages = data.pages;
      // Append only new houses to the container
      const container = document.getElementById('houses-container');
      newHouses.forEach(house => {
        // Create a card using the existing renderHouses logic but without clearing the container
        // We'll reuse the same structure as in renderHouses
...
// Helper to create a single card element (same as in renderHouses)
function createHouseCardElement(house) {
  ...
  // Re-apply slider event listeners (same as in renderHouses)
  const sliderContainer = card.querySelector('.slides-container');
  const dots = card.querySelectorAll('.dot');
  if (sliderContainer && dots.length) {
    let activeIndex = 0;
  ...
  // Rating widget (same as in renderHouses)
  const ratingWidget = card.querySelector('.rating-widget');
  if (ratingWidget) {
    const stars = ratingWidget.querySelectorAll('.star');
    const houseId = ratingWidget.dataset.houseId;
  ...
  // Lightbox on image click (modern touch swipe)
  const slidesContainerLightbox = card.querySelector('.slides-container');
  if (slidesContainerLightbox) {
    slidesContainerLightbox.addEventListener('click', (e) => {
      if (e.target.tagName === 'IMG') {
        e.stopPropagation();
        const currentIdx = Math.round(slidesContainerLightbox.scrollLeft / slidesContainerLightbox.clientWidth);
        openModernLightbox(images, currentIdx);
      }
    });
  }
...
// --- MODERN LIGHTBOX (touch swipe, no arrows, like Airbnb) ---
let currentLightboxImages = [];
let currentLightboxIndex = 0;
let lightboxSlider = null;
let lightboxDotsContainer = null;

function openModernLightbox(images, startIndex = 0) {
  if (!images || images.length === 0) return;
  currentLightboxImages = images;
  currentLightboxIndex = startIndex;
  
  let lightbox = document.getElementById('modernLightbox');
  if (!lightbox) {
    lightbox = document.createElement('div');
    lightbox.id = 'modernLightbox';
    lightbox.className = 'lightbox-gallery';
    lightbox.innerHTML = `
      <div class="close-gallery"><i class="fas fa-times"></i></div>
    ...
    const closeBtn = lightbox.querySelector('.close-gallery');
    ...
    lightbox.addEventListener('touchend', (e) => {
      ...
      if (Math.abs(diff) > 50) {
        ...
        updateModernLightboxSlider();
      }
    });
  }
  lightboxSlider = lightbox.querySelector('.gallery-slider');
  lightboxDotsContainer = lightbox.querySelector('.gallery-dots');
  updateModernLightboxSlider();
  lightbox.classList.add('active');
}

function updateModernLightboxSlider() {
  ...
  currentLightboxImages.forEach((imgSrc, idx) => {
    ...
    dot.addEventListener('click', () => {
      ...
      const slideWidth = lightboxSlider.clientWidth;
      lightboxSlider.scrollTo({ left: idx * slideWidth, behavior: 'smooth' });
      updateModernLightboxDotsAndCounter();
    });
    lightboxDotsContainer.appendChild(dot);
  });
  const slideWidth = lightboxSlider.clientWidth;
  lightboxSlider.scrollTo({ left: currentLightboxIndex * slideWidth, behavior: 'smooth' });
  updateModernLightboxDotsAndCounter();
  lightboxSlider.onscroll = () => {
    const newIndex = Math.round(lightboxSlider.scrollLeft / lightboxSlider.clientWidth);
    if (newIndex !== currentLightboxIndex && newIndex >= 0 && newIndex < currentLightboxImages.length) {
...
function updateModernLightboxDotsAndCounter() {
  const dots = document.querySelectorAll('#modernLightbox .gallery-dot');
  dots.forEach((dot, idx) => {
    if (idx === currentLightboxIndex) dot.classList.add('active');
    else dot.classList.remove('active');
  });
  const counter = document.querySelector('#modernLightbox .gallery-counter');
  if (counter) {
    counter.innerText = `${currentLightboxIndex+1} / ${currentLightboxImages.length}`;
  }
}

// --- RECENTLY VIEWED SECTION (small squared images) ---
...
// --- Instant Preview Tooltip (on hover) ---
let tooltipTimeout;
let currentTooltip = null;
function attachPreviewTooltip(card, house) {
  let timeout;
...
// --- "New" Badge (already in renderHouses, but ensure it works) ---
// The existing code already adds a 'new-badge' class when createdAt is within 24 hours.

// --- Lazy Loading (already added via loading="lazy" in img tags) ---

// --- Remove 3D house border (CSS will be handled in HTML update) ---
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderRecentlyViewed();
  // Attach preview tooltips to existing cards (will be called after each render)
  // We'll hook into the existing renderHouses function to add tooltips to new cards
  const originalRenderHouses = renderHouses;
  window.renderHouses = function(houses) {
    originalRenderHouses(houses);
    // After rendering, attach tooltips to each card
    document.querySelectorAll('.house-card').forEach(card => {
      if (card.dataset.tooltipAttached) return;
      const houseId = card.querySelector('[data-house-id]')?.dataset.houseId || card.querySelector('.slider .slides-container')?.dataset.houseId;
      if (houseId && allHouses) {
        const house = allHouses.find(h => h._id === houseId);
        if (house) {
          attachPreviewTooltip(card, house);
          card.dataset.tooltipAttached = 'true';
        }
      }
    });
  };
  // Also override the createHouseCardElement to attach tooltip
  const originalCreateHouseCardElement = createHouseCardElement;
  window.createHouseCardElement = function(house) {
    const card = originalCreateHouseCardElement(house);
    attachPreviewTooltip(card, house);
    return card;
  };
});